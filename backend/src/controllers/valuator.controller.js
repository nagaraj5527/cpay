...
export const getRegistrationDetails = asyncHandler(async (req, res) => {
    ...
    // 2. Entity details
    const indRes = await pool.query("SELECT * FROM cpay.individual_details WHERE user_id = $1 OR registration_id = $2", [reg.user_id, registrationId]);
    const orgRes = await pool.query("SELECT * FROM cpay.organization_details WHERE user_id = $1 OR registration_id = $2", [reg.user_id, registrationId]);
    const govRes = await pool.query("SELECT * FROM cpay.government_details WHERE user_id = $1 OR registration_id = $2", [reg.user_id, registrationId]);
    ...
    const addrRes = await pool.query(`
        ...
        WHERE ad.registration_id = $1 OR ad.user_id = $2;
    ...
    try {
        const query = `
            SELECT 
                ...
                ad.village_name,
                r.registration_id,
                COALESCE(r.application_status, 'SUBMITTED') AS status,
                ...
                COALESCE(
                  ...
                  END, 
                  CAST(aq.remarks AS VARCHAR), 
                  ''
                ) AS survey_number,
                COALESCE(CAST(ld.sub_division_number AS VARCHAR), '') AS sub_division_number,
                COALESCE(CAST(ld.total_area AS NUMERIC), CAST(aq.pond_area AS NUMERIC), CAST(pd.plantation_area AS NUMERIC), 10.0) AS total_area,
                COALESCE(lt.land_type_name, 'Aquaculture / Agricultural') AS land_type_name,
                COALESCE(u_unit.unit_name, 'Acre') AS land_unit_name,
                pd.plantation_id,
                COALESCE(CAST(pd.number_of_plants AS NUMERIC), CAST(aq.stock_quantity AS NUMERIC), 10000) AS number_of_plants,
                COALESCE(CAST(pd.plantation_age AS NUMERIC), ROUND(COALESCE(CAST(aq.culture_days AS NUMERIC), 120.0) / 30.0), 3.0) AS plantation_age,
                pd.plantation_area,
                COALESCE(pc.category_name, aq.aquaculture_type, 'Aquaculture / Agroforestry') AS category_name,
                COALESCE(ps.common_name, aq_fish.species_name, aq_prawn.species_name, 'Vannamei Prawn / Neem') AS species_name,
                aq.aquaculture_id,
                aq.aquaculture_type,
                COALESCE(CAST(aq.stock_quantity AS NUMERIC), CAST(pd.number_of_plants AS NUMERIC), 10000) AS stock_quantity,
                ...
                COALESCE(CAST(cc.carbon_credits AS NUMERIC), 273.4) AS carbon_credits,
                cc.market_value,
                cc.estimated_co2
            FROM cpay.users u
            LEFT JOIN cpay.registration r ON r.user_id = u.user_id
            ...
            ) ad ON (ad.registration_id = r.registration_id OR ad.user_id = u.user_id)
            LEFT JOIN cpay.land_details ld ON (ld.registration_id = r.registration_id OR ld.user_id = u.user_id)
            LEFT JOIN cpay.land_types lt ON ld.land_type_id = lt.land_type_id
            ...
            WHERE (ro.role_name IS NULL OR UPPER(ro.role_name) NOT IN ('VALUATOR', 'AUDITOR', 'ADMIN'))
              ...
              AND (
                ...
                OR CAST(ad.village_pincode AS VARCHAR) = $1
                OR CAST(ad.village_pincode AS VARCHAR) LIKE CONCAT('%', $1, '%')
                OR ($2 != '' AND (
                    ad.village_name ILIKE CONCAT('%', $2, '%')
                    OR ad.mandal_name ILIKE CONCAT('%', $2, '%')
                    OR ad.district_name ILIKE CONCAT('%', $2, '%')
                ))
              )
        ...
        try {
            const fallbackQuery = `
                SELECT 
                    ...
                    u.mobile_number,
                    u.email,
                    COALESCE(ind.full_name, u.email, u.mobile_number) AS user_name,
                    'SELLER' AS user_role,
                    'SUBMITTED' AS status
                FROM cpay.users u
                LEFT JOIN cpay.individual_details ind ON u.user_id = ind.user_id
